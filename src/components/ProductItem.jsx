...
function ProductItem({product}) {
  const{id, price, category, image, rating:{rate}, title} = product


  return (
      <Link to= {`/products/${id}`}>
      <li
        className="cursor-pointer hover:text-white hover:bg-slate-400 rounded-lg space-y-6 py-4 px-2 bg-slate-200 font-bold"
      >
        <img
